...
def create_app() -> FastAPI:
    app = FastAPI(title=settings.app_name, version=settings.service_version, description="Production API boundary for governed revenue-operations workflows.")
    ...
    async def correlation_and_access_log(request: Request, call_next):
        ...
        logger.info("http request completed", extra={"correlation_id": correlation_id, "method": request.method, "path": request.url.path, "status_code": response.status_code, "duration_ms": round((time.perf_counter() - started) * 1000, 2)})
        return response
    ...
    def evaluate_lead(body: LeadEvaluationRequest, request: Request, idempotency_key: Annotated[str | None, Header(alias="Idempotency-Key")] = None) -> WorkflowResponse:
        return service.process(body, correlation_id=request.state.correlation_id, idempotency_key=idempotency_key)
